...
void NTRU::random_ternary(Poly& poly, int d) {
    ...
    gatherMasterKey(random_bytes, &report); // Use entropy.h's gatherMasterKey
    std::mt19937 gen((random_bytes[0] << 24) | (random_bytes[1] << 16) | (random_bytes[2] << 8) | random_bytes[3]);
